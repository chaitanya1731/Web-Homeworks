...
//let text = '   ';
//let str = '';
//console.log(str.split(/\s+/g).filter(i => i).map(i=>i.length));

//console.log(Array.from(str))//.split(/\s+/g));
    //.filter(i => i).map(i=>i.length))
...
function casedWords(text){
    return(text.toLowerCase().split(/\s+/g).filter(i=>i)
        .map(word => { return (word.length % 2) ? (word.charAt(0).toUpperCase() + word.slice(1))
            : word }));
    ...
    return(text.toLowerCase().split(/\s+/g).filter(i=>i)
        .map(word => { return (word.length % 2) ?
            (word.replace(/[a-z0-9]/gi,
                c=>c[`to${(word=!word)?'Low':'Upp'}erCase`]()))
                : (word.replace(/[a-z0-9]/gi,
                c=>c[`to${(word=!word)?'Upp':'Low'}erCase`]())) }));
}
...
function wordLenPoly(text, x) {
    return (text.split(/\s+/g).filter(i => i).map((i, index) => {
        return (i.length * Math.pow(x, index));})
        .reduce((a, b) => a + b, 0));
}
...
function partialSums(list){
    return (
        list.map((item,index) => list.filter((temp,j) => j <= index))
